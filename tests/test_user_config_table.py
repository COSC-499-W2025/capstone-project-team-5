...
def test_user_config_insert_and_query(db_connection):
    ...
    cursor.execute(
        ...
        (
            ...
            True,
            False,
            json.dumps({"openai": True, "github_api": False}),
            json.dumps(["__pycache__", ".git"]),
        ),
    )
...
def test_user_config_update(db_connection):
    ...
    cursor.execute(
        "INSERT INTO UserConfig (user_config_id, consent_given, use_external_services) "
        "VALUES (1, 0, 0)"
    )

    # Update config
    cursor.execute(
        "UPDATE UserConfig SET consent_given = ?, use_external_services = ? "
    ...
    cursor.execute(
        "SELECT consent_given, use_external_services FROM UserConfig WHERE user_config_id = 1"
    )
    row = cursor.fetchone()
    assert row == (1, 1)
